# ⛏️ AICraft — 方块世界 v0.31

<p align="center">
  <strong>🌍 浏览器中的 3D 体素沙盒 | AI 全自动编写 | 即时可玩</strong>
</p>
...
⚔️ **完整玩法**：8 种武器 · 8 种怪物 · 20+ 合成配方 · 9 个关卡
🎵 **沉浸体验**：程序化 BGM · 15 种音效 · 昼夜循环 · 天气系统（雨/雪/雾）
...
### 方法一：下载 APK（推荐 Android 用户）

**[📥 下载 AICraft v0.31 APK](dist/AICraft.apk)** — 直接安装到 Android 手机，无需任何配置。
...
```bash
npm install
node build.js        # 构建网页版 → dist/AICraft.html
node build.js android # 构建安卓版 → dist/AICraft.apk
```
...
| 改进 | 说明 |
|------|------|
| **🎒 全新背包系统** | 108 格存储 + 8 格快捷栏 + 武器专用栏 + 4 格防具栏，点击交互，触屏友好 |
| **🗺️ 新关卡「烦人的村民」** | 48×48 极紧凑村庄地图，满街 NPC，欢乐喧闹 |
| **📋 任务栏折叠** | 可折叠/展开，不遮挡视野 |
| **🛡️ 武器分类系统** | 武器/防具自动归入专用栏位，快捷栏只放方块和材料 |
| **👾 怪物减半** | 战斗强度更平衡，探索更轻松 |
| **📱 安卓交互优化** | HUD 重叠修复、触屏阈值调整、横屏适配全覆盖 |
| **🌍 9 大关卡** | 烦人的村民 → 草原 → 矿洞 → 沙漠 → 冰原 → 建造天地 → 迷雾沼泽 → 丛林 → 樱花谷 |
| **🎨 关卡视觉差异化** | 每个关卡独特雾效/光照/天气/粒子（雪花、花瓣、雨雾） |
| **🏔️ 红石荒漠生物群系** | 新增 mesa 地形，陶瓦/红砂地表，无绿色植被 |
...
### 🗺️ 关卡系统
- **9 个特色关卡**：烦人的村民、草原起点、矿洞探险、沙漠之旅、冰原探索、建造天地（红石荒漠）、迷雾沼泽、丛林探险、樱花谷
- 每个关卡有独立任务、独特地形、专属雾效和天气
- 切换关卡保留背包和技能
